...
import type { AddGameToLibraryServiceData, LibraryQueryData, UpdateLibraryEntryServiceData } from "../types/library.types.js";
...
export async function getLibraryEntriesController(req: Request, res: Response) {
    const userId = req.userId!
    const libraryQuery = res.locals.query as LibraryQueryData;

    const libraryEntries = await getLibraryEntries(userId, libraryQuery);
